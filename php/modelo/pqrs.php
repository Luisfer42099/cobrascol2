...
class pqrs
{
    private $ced, $nomb, $apel, $tele, $tipo, $corr, $mens, $email;
    private $db;
    ...
    function saveDescarga(){
        $sql = "INSERT INTO Descargas(id, Nombre, Email, Fecha) VALUES (null,'{$this->getNombre()}','{$this->getEmail()}',CURDATE())";
        $save = mysqli_query($this->db, $sql);
        $result = false;
        if ($save) {
            $result = true;
